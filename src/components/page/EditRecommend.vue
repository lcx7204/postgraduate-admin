...
<script>
    ...
    export default {
        ...
        methods:{
            //获取参数
            getParam(){
                var recommendId = this.$route.params.recommendId;
                //获取文章详细信息
                var url = global_.serverUrl;
                var that = this;
                this.$axios.get(url+'/recommend/findRecommendById?recommendId='+recommendId).then(res => {
                    that.recommendId = recommendId;
                    that.recommendTitle = res.data.data.recommendTitle;
                    that.recommendCategory = res.data.data.recommendCategory;
                    that.recommendContent = res.data.data.recommendContent;
                    that.recommendDetail = res.data.data.recommendDetail;
                    that.companyName = res.data.data.companyName;
                    that.author = res.data.data.author;
                    that.price = res.data.data.price;
                    that.dialogImageUrl = res.data.data.recommendIcon;
                    console.log(that.dialogImageUrl);
                    var img = {
                        name:res.data.data.recommendIcon,
                        url:res.data.data.recommendIcon
                    }
                    that.fileList[0] = img;
                });
            },
            ...
            //提交保存
            submit(){
                var recommendId = this.recommendId;
                ...
                var url = global_.serverUrl;
                var that = this;
                this.$axios.post(url+'/recommend/updateRecommend',recommend).then(res => {
                    if (res.data.status==0){
